...
def classify_piece(source_file: str, title: str) -> str:
    ...
    # glasses: لا تعتمد على ملف كامل، فقط العنوان/اسم ملف النظارات المعروف
    ...
    # jewelry: whole word عشان drawstring ما تنحسب ring
    ...
    # scarf / gloves: لا تعتمد على اسم الملف فقط عشان ما تتحول الملابس إلى شال
    if not contains_any_whole_word(t, clothing_words):
    ...
    # dresses first before set, to avoid corset dress being treated as set
    if (
...
def get_relaxed_same_type_only(
    ...
    if color != "أي لون":
        # الالتزام باللون المختار 100% للقطعة المطلوبة
...
def rank_items(piece_type: str, color: str, occasion: str, style: str, height: int, weight: int, body_shape: str) -> pd.DataFrame:
    ...
    # قفل إضافي: لا تسمحي بأي نوع آخر
...
def filter_shoes_by_look(shoes: pd.DataFrame, context: dict) -> pd.DataFrame:
    ...
    if not context["is_winter"]:
        # مع اللوك الصيفي/الخفيف نمنع البوت الطويل والشوز الفرو/الفازي/الصوفي
...
def filter_scarves_by_look(scarves: pd.DataFrame, context: dict) -> pd.DataFrame:
    ...
    if context["is_formal"]:
        # مع السهرة نسمح بالدانتيل/الساتان/الخفيف، ونمنع الثقيل الشتوي
        light = out[(out["is_heavy"] == False) | (out["is_light_fancy"] == True)].copy()
        return light

    # مع اللوك الصيفي/الكاجوال لا نضيف شال/قفاز إلا إذا كان خفيف جدًا وفخم
